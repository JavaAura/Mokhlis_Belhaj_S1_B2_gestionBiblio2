...
import Persistance.reservation.ReservationDAOImpl;
...
public class ConsoleUI {
    ...
    private void modifierUtilisateur() {
        ...
        Utilisateur utilisateur = bibliotheque.trouverUtilisateurParId(id);
        if (utilisateur == null) {
        ...
        if (utilisateur instanceof Professeur) {
            System.out.print("Nouvelle spécialité (laisser vide pour ne pas changer) : ");
            String nouvelleSpecialite = scanner.nextLine();
            if (!nouvelleSpecialite.isEmpty()) {
                ((Professeur) utilisateur).setSpecialite(nouvelleSpecialite);
            }
        } else if (utilisateur instanceof Etudiant) {
            System.out.print("Nouveau matricule (laisser vide pour ne pas changer) : ");
            String nouveauMatricule = scanner.nextLine();
            if (!nouveauMatricule.isEmpty()) {
                ((Etudiant) utilisateur).setMatricule(nouveauMatricule);
            }
        }
    ...
    private void emprunterDocument() {
        ...
        private void retournerDocument() {
            ...
            int documentId = scanner.nextInt();
            scanner.nextLine(); // Consommer le retour de ligne
            boolean retour = bibliotheque.retournerDocument(documentId);
            if(retour){
    ...
    private void reserverDocument() {
        // Déclaration des variables documentId et utilisateurId uniquement dans la méthode
        int documentId = 0;
        int utilisateurId = 0;
        boolean firstAttempt = true;
    
        try {
            // Demander les informations une seule fois
            if (firstAttempt) {
                System.out.print("Entrez l'ID du document à réserver : ");
                documentId = scanner.nextInt();
                System.out.print("Entrez l'ID de l'utilisateur : ");
                utilisateurId = scanner.nextInt();
                scanner.nextLine(); // Consommer le retour à la ligne
                firstAttempt = false; // Marquer que c'est la première entrée
            }
    
            // Utiliser les valeurs des variables sans redemander
            bibliotheque.reserverDocument(documentId, utilisateurId);
        } catch (IllegalArgumentException e) {
            System.out.println("Erreur : " + e.getMessage());
        } catch (IllegalStateException e) {
            System.out.println("Erreur : " + e.getMessage());
            if (e.getMessage().equals("Le document est disponible, tu peux l'emprunter")) {
                System.out.print("Voulez-vous emprunter le document maintenant ? (O/N) : ");
                String reponse = scanner.nextLine();
                if (reponse.equalsIgnoreCase("O")) {
                    try {
                        // Réutiliser documentId et utilisateurId sans redemander
                        bibliotheque.emprunterDocument(documentId, utilisateurId);
                        System.out.println("Document emprunté avec succès.");
                    } catch (Exception ex) {
                        System.out.println("Erreur lors de l'emprunt : " + ex.getMessage());
                    }
                }
            }
    ...
    private void annulerReservation() {
        ...
        scanner.nextLine(); 
        try {
            boolean annule = bibliotheque.annulerReservation(documentId, utilisateurId); 
