...
public class UtilisateurDAOImpl implements UtilisateurDAO {
    ...
    public void modifier(Utilisateur utilisateur) {
        String sqlUtilisateur = "UPDATE Utilisateur SET nom = ?, email = ? WHERE id = ?";
        try (PreparedStatement pstmtUtilisateur = getConnection().prepareStatement(sqlUtilisateur)) {
            pstmtUtilisateur.setString(1, utilisateur.getNom());
            pstmtUtilisateur.setString(2, utilisateur.getEmail());
            pstmtUtilisateur.setInt(3, utilisateur.getId());
            pstmtUtilisateur.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean supprimer(int id) {
        ...
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sqlDeleteUtilisateur)) {
        ...
        try (PreparedStatement pstmt = getConnection().prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (matricule != null) {
                        Etudiant etudiant = new Etudiant(nom, email, matricule);
                        etudiant.setId(id);
                        return etudiant;
                    } else if (specialite != null) {
                        Professeur professeur = new Professeur(nom, email, specialite);
                        professeur.setId(id);
                        return professeur;
                    }
                }
