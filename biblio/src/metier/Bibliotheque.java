...
public class Bibliotheque {
     ...
     * @param reservationDAO DAO pour les opérations sur les réservations.
     */
    public Bibliotheque(DocumentDAO documentDAO, UtilisateurDAO utilisateurDAO, EmpruntDAO empruntDAO, ReservationDAO reservationDAO) {
    ...
    public void reserverDocument(int documentId, int utilisateurId) {
        try {
            ...
            Reservation newReservation = new Reservation(utilisateur, document);
            reservationDAO.ajouterReservation(newReservation);
            System.out.println("Document réservé avec succès.");

        } catch (Exception e) {
            System.err.println("Erreur lors de la réservation : " + e.getMessage());
            // Vous pouvez également logger l'exception ou la gérer d'une autre manière appropriée
        }
    }
     ...
     * @param utilisateurId L'identifiant de l'utilisateur dont la réservation doit être annulée.
     */
    public boolean annulerReservation(int documentId, int utilisateurId) {
        // Vérifier si la réservation existe avant de l'annuler
        if (!reservationDAO.estReserve(documentId)) {
            throw new IllegalStateException("Aucune réservation trouvée pour ce document.");
        }
        return reservationDAO.annulerReservation(documentId, utilisateurId);
    }
